...
export async function main(options: Options = {}) {
  ...
  cli
    ...
    .option('--entry. *<file>', 'Use a ky-value pair as entry files')
    ...
    .option(
      '--watch [path]',
      'Watch mode, if path is specified, it watches the current folder ".". Repeat "--watch" for more than one path'
    )
    ...
    .action(async (files: string[], flags) => {
      ...
      if (flags.dts || flags.distResolve || flags.dtsOnly) {
        options.dts = {};
